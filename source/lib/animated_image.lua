...
function AnimatedImage:init(image_table_path, options)
    options = options or {}
    ...
    if options.sequence ~= nil then
        ...
        if sequence_image_table == nil then
            error("Failed to create sequence image table")
        end
    ...
    if animation_loop == nil then
        error("Failed to create animation loop")
    end
